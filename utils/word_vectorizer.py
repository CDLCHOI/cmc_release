...
class WordVectorizer(object):
    def __init__(self, meta_root, prefix):
        vectors = np.load(pjoin(meta_root, '%s_data.npy'%prefix)) # glove/our_vab_data.npy
        words = pickle.load(open(pjoin(meta_root, '%s_words.pkl'%prefix), 'rb')) # glove/our_vab_words.pkl
        self.word2idx = pickle.load(open(pjoin(meta_root, '%s_idx.pkl'%prefix), 'rb')) # glove/our_vab_idx.pkl
